...
function SpriteSheet(n, imgref, f, fsize, forigin)
{
	...
	this.DrawFrame = function(p, fr, /*scale,*/ context)
	{
		var pos = {x:Math.floor(p.x), y:Math.floor(p.y)}
		fr = Math.floor(fr);
		if (fr >= this.frameCount) fr = fr % this.frameCount;
		context.drawImage(this.image, Math.floor(fr * this.frameSize.x), 0, this.frameSize.x, this.frameSize.y, pos.x - this.frameOrigin.x - g_spriteCamera.x, pos.y - this.frameOrigin.y - g_spriteCamera.y, this.frameSize.x, this.frameSize.y);
	}
}
...
function DrawSpriteQueue(context)
{
	...
	{
		g_SpriteSheetList[g_SpriteDrawQueue[i].spriteIndex].DrawFrame(g_SpriteDrawQueue[i].position, g_SpriteDrawQueue[i].frame, context);
	}
	// Clear the queue.
	ClearSpriteQueue();
}
