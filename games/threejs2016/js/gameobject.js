...
//**********************************************************************************************************************
// Game object closure:
function GameObject( pIndex, pHandler, pGivenApp )
{
	// General variables:
	var index = pIndex;
	this.active = false;
	...
	this.collisionRect = {min:{x:0, y:0, z:0}, max:{x:0, y:0, z:0}};
	this.collision = {min:{x:0, y:0, z:0}, max:{x:0, y:0, z:0}};
	...
	this.init = function(pType, pModule, pPosition)
	{
		...
		this.type = pType;
		this.position.x = pPosition.x; this.position.y = pPosition.y; this.position.z = pPosition.z;
		this.velocity.x = 0; this.velocity.y = 0; this.velocity.z = 0;
		
		this.collision.min.x = this.position.x + this.collisionRect.min.x;
		this.collision.min.y = this.position.y + this.collisionRect.min.y;
		this.collision.min.z = this.position.z + this.collisionRect.min.z;
		this.collision.max.x = this.position.x + this.collisionRect.max.x;
		this.collision.max.y = this.position.y + this.collisionRect.max.y;
		this.collision.max.z = this.position.z + this.collisionRect.max.z;
	...
	this.update = function()
	{
		...
		this.position.x += this.velocity.x * 1 / GameEngineConstants.fps;
		this.position.y += this.velocity.y * 1 / GameEngineConstants.fps;
		this.position.z += this.velocity.z * 1 / GameEngineConstants.fps;
		
		// If the position changed, update our collision area.
		if (this.position.x != this.prevPos.x || this.position.y != this.prevPos.y || this.position.z != this.prevPos.z || collisionChanged)
		{
			this.updateCollisionArea();
		}
	}
	...
	this.collide = function(pOther)
	{
		...
		if (collideAABBs(this.collision, pOther.collision))
		{
			if (this.module != null)
			{
